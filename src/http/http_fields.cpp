...
struct field_table
{
    static std::uint32_t get_chars(unsigned char const* p) noexcept
    {
    ...
    static bool equals(string_view lhs, string_view rhs)
    {
...
// string constants
    ...
    HTTPField string_to_field(string_view s) const
    {
        auto h = digest(s);
        auto j = h % N;
        int i = map_[j][0];
...
field_table const& get_http_field_table()
{
...
std::string_view ToString(HTTPField f)
{
    auto const& v = get_http_field_table();
    assert(static_cast<unsigned>(f) < v.size());
    return v.begin()[static_cast<unsigned>(f)];
}


std::ostream& AsyncRuntime::operator << (std::ostream& os, HTTPField f)
{
    auto const& v = get_http_field_table();
    assert(static_cast<unsigned>(f) < v.size());
    return os << v.begin()[static_cast<unsigned>(f)];
}